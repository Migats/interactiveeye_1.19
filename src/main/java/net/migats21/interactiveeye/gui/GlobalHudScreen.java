...
public abstract class GlobalHudScreen extends GuiComponent {
    ...
    private static ModChatComponentScreen chatScreen;
    ...
    public static void init() {
        ...
        chatScreen = new ModChatComponentScreen();
    }

    public static void renderAll(PoseStack poseStack, float deltaFrameTime) {
        int width = minecraft.getWindow().getGuiScaledWidth();
        int height = minecraft.getWindow().getGuiScaledHeight();
        chatScreen.render(poseStack, deltaFrameTime, width, height);
    ...
    protected static float bezierCurve(float t, float c0, float c1, float c2, float c3) {
        return t*t*t*(c0+3.0f*c1-3.0f*c2+c3) + t*t*(3.0f*c0-6.0f*c1+3.0f*c2)+t*(-3.0f*c0+3.0f*c1)+c0;
    }
    ...
    protected void renderBackground(PoseStack poseStack, int x, int y, int width, int height) {
        ...
        fill(poseStack, x, y, x + width, y + height, minecraft.screen instanceof AbstractContainerScreen<?> ? 0xe00f2e11 : 0x40458a48);
